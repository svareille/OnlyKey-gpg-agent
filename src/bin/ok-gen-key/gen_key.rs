...
pub(crate) fn gen_key(
    ...
    let verifying_key = onlykey
        .pubkey(&sign_key_info)
        .context("Could not get the verifying public key")?;
    ...
    let encryption_key = onlykey
        .pubkey(&decrypt_key_info)
        .context("Could not get the encryption public key")?;

    // Generating a [Transferable Public Key](https://www.rfc-editor.org/rfc/rfc4880#section-11.1)

    let primary_key: Key<PublicParts, PrimaryRole> = Key::from(
        Key4::new(
            creation,
            match key_kind {
            ...
            match key_kind {
                EccKind::Ed25519 => PublicKey::EdDSA {
                    curve: Curve::Ed25519,
                    q: MPI::new_compressed_point(&verifying_key),
                },
                EccKind::Nist256 => PublicKey::ECDSA {
                    curve: Curve::NistP256,
                    q: MPI::new_point(&verifying_key[..32], &verifying_key[32..], 256),
                },
                EccKind::Secp256 => {
                    unimplemented!("Secp256k1 curve is currently not supported by sequoia")
                }
            },
        )
    ...
    let mut public_key = Cert::try_from(Packet::PublicKey(primary_key.clone()))
        .context("Failed to generate certificate from primary key")?;
    ...
    let uid_sig_builder = SignatureBuilder::new(SignatureType::PositiveCertification)
        ...
        .and_then(|b| {
            b.set_preferred_symmetric_algorithms(vec![
        ...
        .and_then(|b| {
            b.set_preferred_hash_algorithms(vec![
        ...
        .and_then(|b| {
            b.set_preferred_compression_algorithms(vec![
    ...
    let subkey: Key<PublicParts, SubordinateRole> = Key::from(
        Key4::new(
            ...
            match key_kind {
                EccKind::Ed25519 => PublicKey::ECDH {
                    curve: Curve::Ed25519,
                    q: MPI::new_compressed_point(&encryption_key),
                    hash: HashAlgorithm::SHA512,
                    sym: SymmetricAlgorithm::AES256,
                },
                EccKind::Nist256 => PublicKey::ECDH {
                    curve: Curve::NistP256,
                    q: MPI::new_point(&encryption_key[..32], &encryption_key[32..], 256),
                    hash: HashAlgorithm::SHA512,
                    sym: SymmetricAlgorithm::AES256,
                },
                EccKind::Secp256 => {
    ...
    let subkey_signature = subkey_sig_builder
    ...
    let armored = String::from_utf8(public_key.armored().to_vec().unwrap())
        .context("Failed to armor the cert")?;
    Ok(armored)
}
...
impl Signer for OnlyKeySigner {
    ...
    fn sign(
        ...
        let (b1, b2, b3) =
            OnlyKey::compute_challenge(&OnlyKey::data_to_send(digest, &self.key_info));
        ...
        match self.key_info.r#type() {
            ...
            ok_gpg_agent::config::KeyType::Ecc(ecc) => match ecc {
                ...
                EccType::Ed25519 => Ok(sequoia_openpgp::crypto::mpi::Signature::EdDSA {
                    r: MPI::new(&signature[0..32]),
                    s: MPI::new(&signature[32..64]),
                }),
                EccType::Cv25519 => unreachable!(),
                EccType::Nist256P1 => Ok(sequoia_openpgp::crypto::mpi::Signature::ECDSA {
                    r: MPI::new(&signature[0..32]),
                    s: MPI::new(&signature[32..64]),
                }),
                EccType::Secp256K1 => Ok(sequoia_openpgp::crypto::mpi::Signature::ECDSA {
                    r: MPI::new(&signature[0..32]),
                    s: MPI::new(&signature[32..64]),
                }),
            },
